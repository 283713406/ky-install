...
function check_installer_ok(){
    ...
    kubectl -n kylincloud-system wait --timeout=180s --for=condition=Ready $(kubectl -n kylincloud-system get pod -l app=ks-install -oname)
    echo "waiting for KylinCloud ready"
    while IFS= read -r line; do
        echo $line
        if [[ $line =~ "Welcome to KylinCloud" ]]
            then
                return
        fi
    done < <(timeout 1200 kubectl logs -n kylincloud-system deploy/ks-installer -f)
    echo "ks-install not output 'Welcome to KylinCloud'"
    exit 1
}
