# Configurable Parameters in KylinCloud


installer configuration, used when install a KylinCloud
```yaml
persistence:
  storageClass: ""
etcd:
...
common:
  ...
  redisVolumSize: 2Gi
  monitoring:
    endpoint: http://prometheus-operated.kylincloud-monitoring-system.svc:9090
console:
...
multicluster:
  # host: means installer will install current cluster as a Host Cluster. There should be only one HOST CLUSTER in KylinCloud multicluster.
