...
[kube-events](https://github.com/kylincloud/kube-events) is an integration for Kubernetes Event exporting, filtering and alerting.
...
To install the chart with the release name `kube-events` into the namespace `kylincloud-logging-system`:

```console
helm install kube-events ./ --namespace kylincloud-logging-system
```
...
To uninstall/delete the `kube-events` release in the namespace `kylincloud-logging-system`:

```console
helm delete kube-events --namespace kylincloud-logging-system
```
...
```console
kubectl delete crd rulers.events.kylincloud.io
kubectl delete crd exporters.events.kylincloud.io
kubectl delete crd rules.events.kylincloud.io
```
...
| operator.image.repository | Repository for kube-events-operator image | `kylinclouddev/kube-events-operator` |
...
| exporter.image.repository | Repository for kube-events-operator image | `kylinclouddev/kube-events-exporter` |
...
| ruler.image.repository | Repository for kube-events-ruler image | `kylinclouddev/kube-events-ruler` |
...
| ruler.sinks.alertmanager.namespace | Namespace of alertmanager service | `kylincloud-monitoring-system` |
