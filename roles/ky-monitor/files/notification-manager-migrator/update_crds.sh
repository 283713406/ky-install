...
update_v1alpha1() {
  ...
  for file in $files; do
    ...
    resource=$(echo "$src" | jq 'setpath(["apiVersion"]; "notification.kylincloud.io/v2beta1")')
...
version=$(kubectl get crd notificationmanagers.notification.kylincloud.io -o jsonpath='{.spec.versions[0].name}')
