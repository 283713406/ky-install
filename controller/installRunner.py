...
configFile = '/kylincloud/config/ky-config.json'
statusFile = '/kylincloud/config/ky-status.json'
...
cluster_configuration = {
    "apiVersion": "installer.kylincloud.io/v1alpha1",
    "kind": "ClusterConfiguration",
    "metadata": {
        "name": "ky-installer",
...
def preInstallTasks():
    ...
    preInstallTasks['ky-core'] = [
        os.path.join(playbookBasePath, 'ky-core.yaml'),
        os.path.join(privateDataDir, 'ky-core')
    ]
...
def resultInfo(resultState=False, api=None):
    ks_config = ansible_runner.run(
        playbook=os.path.join(playbookBasePath, 'ky-config.yaml'),
        private_data_dir=privateDataDir,
        artifact_dir=os.path.join(privateDataDir, 'ky-config'),
        ident='ky-config',
        quiet=True
    )

    if ks_config.rc != 0:
        print("Failed to ansible-playbook ky-config.yaml")
        exit()
    ...
    if "migration" in resource['status']['core'] and resource['status']['core']['migration'] and resultState == False:
        migration = ansible_runner.run(
            playbook=os.path.join(playbookBasePath, 'ky-migration.yaml'),
            private_data_dir=privateDataDir,
            artifact_dir=os.path.join(privateDataDir, 'ky-migration'),
            ident='ky-migration',
            quiet=False
        )
        if migration.rc != 0:
